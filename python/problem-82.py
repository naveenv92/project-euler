...
grid = np.array([[131, 673, 234, 103, 18], [201, 96, 342, 965, 150], [630, 803, 746, 422, 111], [537, 699, 497, 121, 956], [805, 732, 524, 37, 331]])
...
for i in range(len(grid) - 2, -1, -1):
	for j in range(len(grid) - 3, 0, -1):
		if i == 0:
			...
			newgrid[i, j] += min(grid[i - 1, j], grid[i, j + 1], grid[i + 1, j])
